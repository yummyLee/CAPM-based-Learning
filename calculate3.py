...
parser.add_argument('-rs', type=bool, default=False, help='metric_type')
parser.add_argument('-rt', type=bool, default=False, help='metric_type')
...
def cal_middle_output(m_model, arch, input_tensor, layer_index):
    ...
    if arch == 'alexnet' or arch == 'm_alexnet':
        ...
        # elif layer_index == 6:
        #     input_flat = input_tensor.view(-1, 9216)
...
def do_cal_jsd_list_between_tensors(param):
    # print('=====do=====', param[2])
    cal_jsd_list_between_tensors(param[0], param[1], param[2], param[3], param[4])


def do_cal_jsd_list_between_tensors3(param):
    # print('=====do=====', param[2])
    t0 = get_min_pool_npy(param[0], '', param[5], param[6], param[7], param[8], param[10], param[11])
...
def cal_jsd_list_between_tensors(tensor_a, tensor_b, save_path, save_path2, mt):
    """
    :param mt:
    :param save_path2: store the abs d file
    :param tensor_a: dim: 200(1080)*9216
    :param tensor_b: dim: 200(1080)*9216
    ...
    if mt == 'wa':
        for i in range(min(tensor_a.shape[1], tensor_b.shape[1])):
            ...
            index_arr = np.array(list(range(len(tensor_a[:, i]))))
            # print(index_arr.shape, tensor_a[:, i].shape)

            metric = 0
            try:

                metric = wasserstein_distance(index_arr, index_arr, tensor_a[:, i] + 0.0000001,
                                              tensor_b[:, i] + 0.0000001)
            except ValueError:
                print(i)
                print(tensor_a[:, i])
                print(tensor_b[:, i])
...
def get_min_pool_accuracy(img_dir, class_index, deleted_node_set, keep_node_pools, keep_node_pools_weight, arch,
    ...
    keep_node_set = np.array(list(set(list(range(9216))) - set(deleted_node_set)))
...
def main():
                    ...
                    for transform_class_item in os.listdir(img_dir + '/' + class_item):
                        ...
                        params.append(
                            (
                                ...
                                model))
                        get_layer(img_dir + '/' + class_item + '/' + transform_class_item, arch, args.l, save_dir_pre,
                                  model)
            ...
            for test_layer in range(args.l, args.l + 1):
                ...
                if mode == 'zone':
                    ...
                    for transform_img_index2 in iter_list:
                        ...
                        for transform_img_index_cp in range_list_inner[0:len(range_list_inner)]:

                            if transform_img_index_cp == range_list_inner[compare_index]:
                                continue

                            count += 1

                            train_dir = img_dir + '/' + multi_class_item + '/' + transform_img_index_cp
                            train_dir = train_dir.replace('/', '-')
                            ...
                            metric_list_np_name = metric_list_np_dir + '/metric_list_%s-%s_%sd_%s_%s_%d_zero_' % (
                                transform_img_index2, transform_img_index_cp, args.mt, ts_operation, mode,
                                args.l) + transform_img_index_cp + '.npy'

                            abs_d_list_np_name = abs_d_list_np_dir + '/metric_list_%s-%s_abs_d_%s_%s_%d_zero_' % (
                                transform_img_index2, transform_img_index_cp, ts_operation, mode,
                                args.l) + transform_img_index_cp + '.npy'
    ...
    if args.op == 'cal_pools_jsd_marker_cd_zone_time_zero_inner_all':
        ...
        for multi_class_item in multi_classes:
            ...
            for test_layer in range(args.l, args.l + 1):
                ...
                for transform_img_index_inner in range_list_inner:
                    # print(range_list)
                    ...
                    for multi_class_item_cp in multi_classes:
                        ...
                        for transform_img_index_cp in range_list_cp[0:len(range_list_cp)]:

                            if transform_img_index_cp == range_list_cp[compare_index]:
                                continue

                            # if transform_img_index_cp != 'n03937543_4835_sc':
                            #     continue
                            ...
                            train_dir = img_dir + '/' + multi_class_item_cp + '/' + transform_img_index_cp
                            train_dir = train_dir.replace('/', '-')
    ...
    if args.op == 'cal_jsd_marker_cd_zone_time_zero_other_inner_all':
        ...
        for multi_class_item in multi_classes:
            ...
            for multi_class_item_cp in multi_classes:
                ...
                if multi_class_item_cp == 'n00000000' or multi_class_item == 'n00000000' or multi_class_item == multi_class_item_cp:
                    continue
                ...
                for test_layer in range(args.l, args.l + 1):
                    ...
                    for transform_img_index_inner in range_list_inner[0:len(range_list_inner)]:
                        ...
                        for transform_img_index_cp in range_list_inner[0:len(range_list_cp)]:
                            ...
                            try:
                                ...
                                print('file_not_found: ', transform_img_index_cp)
                                continue
                            ...
                            metric_list_np_name = metric_list_np_dir + '/metric_list_o_cpt_%s-%s-%s-%s_%sd_%s_%s_%d_zero_' % (
                                transform_img_index_inner, multi_class_item, multi_class_item_cp,
                                transform_img_index_cp, args.mt, ts_operation,
                                mode,
                                args.l) + '.npy'

                            abs_d_list_np_name = abs_d_list_np_dir + '/metric_list_o_cpt_%s-%s-%s-%s_abs_d_%s_%s_%d_zero_' % (
                                transform_img_index_inner, multi_class_item, multi_class_item_cp,
                                transform_img_index_cp, ts_operation, mode,
                                args.l) + '.npy'
        ...
        for multi_class_item in multi_classes:
            ...
            for test_layer in range(args.l, args.l + 1):
                ...
                for seq_index in range(1, t_num):
                    ...
                    tensor_array_list[0] = tensor_array_list[1]
    ...
    if args.op == 'analysis_jsd_marker_c_zone_zero2':
        ...
        for multi_class_item in os.listdir(origin_image_dir):
            ...
            for transform_img_index2 in iter_list:
                ...
                for metric_file in metric_file_name_list:
                    if not metric_file.__contains__(transform_img_index2 + '-'):
                        continue
                    metric_file_np = np.load(metric_list_np_dir + '/' + metric_file)
                    # print(metric_file_np.shape)
                    metric_file_list2.append(metric_file_np.reshape(metric_file_np.shape[0], ).tolist())
        ...
        for multi_class_item in os.listdir(origin_image_dir):
            ...
            for multi_class_item_cp in os.listdir(origin_image_dir):
                ...
                if args.ioi == 'o':
                    ...
                    for transform_index_inner in range_list_inner:
            ...
            for multi_class_item_cp in os.listdir(origin_image_dir):
                ...
                for transform_index_inner in range_list_inner:
                    ...
                    for transform_index_cp in range_list_cp:
                        ...
                        metric_list_np_name = metric_list_np_dir + '/metric_list_o_cpt_%s-%s-%s-%s_%sd_%s_%s_%d_zero_' % (
                            transform_index_inner, multi_class_item, multi_class_item_cp,
    ...
    if args.op == 'analysis_deleted_node2':
        ...
        for multi_class_item in os.listdir(t + '/' + args.arch + '/metric_list_npy_dir'):
            ...
            if args.dnop.__contains__('cov'):
                ...
                deleted_node_marker_cov_set = set(list(range(9216))) - set(deleted_node_marker_cov_set_pre)
            ...
            for inner_index in range(all_o_metric_file_list.shape[0]):
                ...
                for i in keep_node_set:
                    ...
                    io_metric_file_list_del_index.append(
                        (o_metric_file_list[:, i] + constant_plus) / ((metric_file_list[:, i] + constant_plus)))
    ...
    if args.op == 'analysis_zzc_cov':
        ...
        for multi_class_item in os.listdir(origin_image_dir):
            ...
            r_i_list_topk_value, r_i_list_topk_index = torch.from_numpy(er_list).topk(
                4 * int(np.sqrt(len(keep_node_set))))
            ...
            for j in range(len(keep_node_set)):
                ...
                ltopk_v, htopk_index = torch.from_numpy(log_metric_cof[j, r_i_list_topk_index]).topk(10,
                                                                                                     largest=False)
                ltopk_v, ltopk_index = torch.from_numpy(np.abs(log_metric_cof[j, r_i_list_topk_index])).topk(10,
                                                                                                             largest=False)
    ...
    if args.op == 'cal_io_big_node':
        ...
        for multi_class_item in os.listdir(origin_image_dir):
            ...
            #     iter_list = range_list_inner[0:len(range_list_inner)]
            #
            # for transform_img_index_cp in iter_list:
            #
            #     if transform_img_index_cp == range_list_inner[compare_index]:
            ...
            #     i_wa_npy = np.load(
            #         t + '/' + args.arch + '/metric_list_npy_dir/' + multi_class_item + '/wa_class_npy/%s%s_file_list_%s-%s_%s_%s_%s.npy' % (
            #             args.mt, time_type, multi_class_item, transform_img_index_cp, ts_operation, mode, phase))
            #     # print(i_wa_npy.shape)
            #     for index in keep_node_set:
            #         # print(index)
            #         if np.mean(i_wa_npy[:, index]) < np.mean(o_wa_npy[:, index]):
