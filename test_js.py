...
# for i in range(0,1):
    ...
    # print(y[i].shape)
    # print(y[int(data_list_len) + i].shape)
    # axs[i].scatter(x, y[i], color='red')
    # axs[i].scatter(x, y[int(data_list_len) + i], color='blue')
    # axs[i].plot(x, y[i], color='black', linestyle='--')
    # axs[i].plot(x, y[int(data_list_len) + i], color='black')
    s1[i] = axs[i].plot(x[0:200], y[i][0:200], color='blue', linestyle='--', label='组合前')
    s2[i] = axs[i].plot(x[0:200], y[int(data_list_len) + i][0:200], color='black', linestyle='-', label='组合后')
    # axs[i].set_xlabel('')
    axs[i].set_ylabel('收益', fontsize=21)
    axs[i].set_xlabel('节点编号', fontsize=21)
...
# axs[0].set_title('(a)节点' + str(test_index[0]) + '组合前', y=-0.15, fontsize=21)
# axs[1].set_title('(b)节点' + str(test_index[0]) + '组合后', y=-0.15, fontsize=21)
